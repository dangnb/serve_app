...
var gettingNewTabUrl = new Promise(function (resolve) {
    extensionStateStorage.get()
        .then(function (state) {
        ...
        var assistKey = "assistNT";
        var oldAssistKey = "assist";
        if (!window.localStorage.getItem(assistKey) && !window.localStorage.getItem(oldAssistKey) && !~document.location.href.indexOf("?")) {
            window.localStorage.setItem(assistKey, "1");
            if (PageUtils.isDoorHangerDisplayed()) {
                newTabUrl = PageUtils.appendParamsToShowAssist(newTabUrl);
                state.assistNT = true;
                extensionStateStorage.set(state).then(Logger.warn);
            }
        }
...
if (BrowserUtils.isAPIAvailable().tabsExecuteScriptFrameIdSupport) {
    ...
    gettingNewTabUrl.then(function (url) {
