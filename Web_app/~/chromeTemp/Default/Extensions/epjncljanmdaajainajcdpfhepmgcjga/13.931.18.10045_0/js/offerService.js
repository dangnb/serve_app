...
var OfferService = (function () {
    function OfferService() {
        ...
        this.exec = function (extensionConfig) {
            ...
            var elapsed = DateTimeUtils.timer();
            AJAX.readConfigJSON(_this.extensionConfig.buildVars.offerServiceConfigUrl)
                .catch(function (err) {
        ...
        this.logInfoUL = function (infoSpecificData) {
            ...
            if (infoSpecificData.message === "on-error") {
                ask.apps.ul.fireErrorEvent(_this.extensionConfig.buildVars.unifiedLoggingUrl, infoSpecificData, _this.extensionConfig, "OfferService").catch(Logger.warn);
            }
            else {
                ask.apps.ul.fireInfoEvent(_this.extensionConfig.buildVars.unifiedLoggingUrl, infoSpecificData, _this.extensionConfig, "OfferService").catch(Logger.warn);
            }
        };
        ...
        this.checkOfferService = function () {
            if (!_this.config.offerServiceSettings.serviceURL) {
                ...
                return makeAjaxRequest()
                    ...
                    if (response && response.data && response.data.offerInfo && response.data.offerInfo.offerURL) {
                        _this.state.offerURL = response.data.offerInfo.offerURL;
                        _this.state.redirectToWTT = response.data.offerInfo.noRedirect !== "true";
                    }
                    else {
                        delete _this.state.offerURL;
                        delete _this.state.redirectToWTT;
